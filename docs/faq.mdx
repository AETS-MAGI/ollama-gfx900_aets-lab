...
## Why is Flash Attention disabled on my gfx900 / MI25?

Flash Attention is intentionally disabled for gfx900 in this fork (`ml/device.go:FlashAttentionSupported()`). Setting `OLLAMA_FLASH_ATTENTION=1` has no effect on gfx900. This is a stability measure — gfx900's MLIR iGEMM support was disabled upstream in 2021, and Flash Attention depends on code paths that are not reliable on this architecture.

## Why is my context window limited to 8192 tokens on gfx900?

This fork caps the default `num_ctx` at 8192 for gfx900 (`server/routes.go`) to prevent VRAM exhaustion on MI25 (16 GB HBM2). You can override it with `OLLAMA_CONTEXT_LENGTH`, but stability above 8192 tokens is not guaranteed on this hardware.
