...
> **Fork note:** This FAQ includes gfx900/MI25-specific entries in addition to upstream content.

## Is gfx900 / MI25 (Radeon Instinct) supported?

gfx900 is **not supported by upstream Ollama** (blocked in commit `55ca82726`). This fork removes that block and adds gfx900-specific stability patches.

**Verified in this fork:**
- MI25/gfx900 recognized by ROCm at `rocminfo` level
- GPU inference confirmed with tinyllama and deepseek-r1:14b after local rocBLAS path injection
- GPU discovery progressed from immediate CPU fallback after fixes

**Still unconfirmed:** broad model compatibility, multi-session stability, portability across machines.

See [gfx900 / MI25 Guide](./gfx900-mi25.md) for build and setup instructions.

## Why is Flash Attention not available on my gfx900 / MI25?

`FlashAttentionSupported()` in `ml/device.go` returns `false` when a gfx900 device is present, so Flash Attention is not used regardless of the `OLLAMA_FLASH_ATTENTION` setting. The server log will show `FlashAttention:Auto` (the initial parameter type) — this is expected; it means the user has not overridden it, but since `FlashAttentionSupported()` returned false, it is not activated.

Background: gfx900's MLIR iGEMM support was disabled upstream in 2021, and the code paths that Flash Attention depends on are not reliable on this architecture.

## Why does `num_ctx` seem limited on gfx900?

The server calculates a VRAM-based default `num_ctx` at startup. If that default exceeds 8192 and a gfx900 device is present, it is capped to 8192 (`server/routes.go`). On MI25 (16 GB HBM2), the VRAM-based default is typically 4096, so the cap does not apply — the default context is 4096, not 8192.

You can override it with `OLLAMA_CONTEXT_LENGTH`, but stability above 8192 tokens is not guaranteed on MI25.

## My gfx900 model runs on CPU. What do I do?

The most common cause is a missing gfx900-specific rocBLAS library. See the [Troubleshooting guide — gfx900 section](./troubleshooting.mdx#gfx900--mi25-troubleshooting) for a step-by-step diagnosis.

Short checklist:
1. `rocminfo | grep gfx` — device must be detected
2. `ls build-gfx900/lib/ollama/libggml-hip.so` — build artifact must exist
3. If rocBLAS crashes: build local rocBLAS with `./build-rocblas-gfx900.sh`
