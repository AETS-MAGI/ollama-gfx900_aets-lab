...
> **Fork note:** This document covers hardware support for the gfx900/MI25 optimization fork.
> See [gfx900 / MI25 Guide](./gfx900-mi25.md) for build and runtime details specific to gfx900.
> [日本語版はこちら](./gpu.ja.mdx)
...
### gfx900 / MI25 (This Fork)

This fork adds explicit support for AMD Vega / gfx900 / Radeon Instinct MI25.
gfx900 is **not included in the upstream Ollama AMD support table** and requires
a fork-specific build. The following behaviors are enforced in code for gfx900:

| Behavior | Value | Source |
|----------|-------|--------|
| Flash Attention | Not used — `FlashAttentionSupported()` returns `false`; `OLLAMA_FLASH_ATTENTION` has no effect | `ml/device.go` |
| Default `num_parallel` | 1 (forced) | `server/sched.go` |
| `num_ctx` ceiling | 8192 — only applies if VRAM-based default exceeds 8192; on MI25 (16 GB) typical default is 4096 | `server/routes.go` |
| Build output directory | `build-gfx900/lib/ollama` | `ml/path.go` (priority search) |

See [gfx900 / MI25 Guide](./gfx900-mi25.md) for full build and runtime instructions.
